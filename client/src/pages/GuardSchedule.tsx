...
import { guardShifts, type GuardShift } from '@/lib/mockData';
...
const statusConfig: Record<string, { label: string; badge: string; dot: string }> = {
  active: { label: '值班中', badge: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25', dot: 'online' },
  break: { label: '休息中', badge: 'bg-amber-500/15 text-amber-400 border-amber-500/25', dot: 'warning' },
  off: { label: '休班', badge: 'bg-slate-500/15 text-slate-400 border-slate-500/25', dot: 'offline' },
};
    ...
    <AdminLayout title="保安排班">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={() => toast.success('新增保安', { description: '功能开发中' })}
                ...
                <div className="p-3 flex flex-col gap-2">
                  {guards.map(guard => {
                    ...
                    return (
                      ...
                      >
                        <div className="w-9 h-9 rounded-xl bg-sky-500/15 border border-sky-500/25 flex items-center justify-center text-xs font-bold text-sky-400 font-display shrink-0">
                          {guard.avatar}
                        </div>
              ...
              {guardShifts.map(guard => {
                ...
                return (
                  <tr key={guard.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-400">{shiftLabel[guard.shift]}</td>
                    <td className="px-4 py-3">
